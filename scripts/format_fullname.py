...
def formatName(name):
   ...
   middlename = splitname.pop(0) if len(splitname) > 1 else '';
   if len(splitname) > 1:
      middlename = splitname.pop(0)
   
   # return [ <titles>, <firstname>, <middlename>, <lastname> ]
   return [
      ', '.join(sorted(fulltitle)) if fulltitle else '',
	  firstname.capitalize(),
	  middlename.capitalize(),
	  ' '.join(splitname).title()
   ]
   
print formatName("MD Muhammad A Khan")
print formatName("Negar Khaefi")
print formatName("Leah Gaedeke - Bc FNP")
print formatName("MA LCDC LPC LM&FT Fred Lee Richardson")
